 ...
 * Initializes tables and creates seed data for QuizFlow SaaS
 */
...
async function createSeedData() {
  try {
    ...
    const existingUser = await sql`SELECT id FROM users WHERE email = 'demo@quizflow.com'`;
    ...
    for (const leadData of sampleLeads) {
      ...
      await sql`UPDATE quizzes SET leads_count = leads_count + 1 WHERE id = ${quizId}`;
    }
...
async function initializeDatabase() {
  console.log('🚀 Initializing QuizFlow Database...');
  ...
  try {
    ...
    console.log('- Demo User: ✅ Available (demo@quizflow.com / demo123)');
    ...
    console.error('❌ Database initialization failed:', error);
    console.error('Make sure POSTGRES_URL environment variable is set');
    process.exit(1);
  }
}
